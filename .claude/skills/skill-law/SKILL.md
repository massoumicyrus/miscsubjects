...
6. Judge the edit: fresh agent, baseline-fails then with-skill-passes; link both exhibits.
7. Record actor, exhibit, and delta in the ledger; the skill points at the receipt.
8. Project the skill to its site object so every model reads the same law from one identity.
...
Return only: the exhibit, the clause delta, the gate (if mechanical), the fresh-agent verdict, the ledger receipt, both trees synced. Cite kao:skill-law, its version, and the exhibits used.
