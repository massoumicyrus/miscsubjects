# loop-safe-storage — the storage agent
...
npx wrangler pages secret put STORE_KEY --project-name loop-safe-miscsubjects   # paste same value
...
git commit -m "move reference docs into loop-safe-storage; drop from repo"
```
