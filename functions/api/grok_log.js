// Ingest one Grok CLI turn from the Stop hook (hooks/grok-turn-log.js) into grok_turns + agent_turns.
...
export async function onRequestPost(context) {
  ...
  try {
    ...
    if (turnKey) {
      const dup = await env.DB.prepare('SELECT id FROM grok_turns WHERE turn_key = ? LIMIT 1').bind(turnKey).first();
      if (dup) {
        try {
    ...
    const ins = await env.DB.prepare(
      `INSERT INTO grok_turns (ts, session, cwd, input_kind, user_input, user_input_chars, assistant_text, n_tools, tools_json, commands_json, files_json, system_prompt, turn_key)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
    ).bind(
      String(buildNowIso()),
      String(r.session || ''),
