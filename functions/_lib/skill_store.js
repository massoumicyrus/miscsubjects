// SKILLS AS FIRST-CLASS OBJECTS — the D1 store behind /api/skills.
//
// THE DEFECT THIS CURES (SPEC_SKILL_EVIDENCE_GRAPH.md, Phase 1). Until this module, a skill was a
// build-time constant: functions/_lib/skill_registry.js, regenerated by hand, last on 2026-08-12.
// A skill had no version, no hash and no history, so nothing on the build could truthfully say
// WHICH method text governed an execution — which makes reproduction and comparative method
// testing structurally impossible. Articles solved this long ago (body_hash + expected_hash CAS);
// this applies the same discipline to methods.
//
// DIVISION OF AUTHORITY, deliberately one-directional:
//   .claude/skills/<name>/SKILL.md   what agents LOAD — the filesystem stays canonical for use
//   skill_objects / skill_versions   what the site SERVES AND CITES — receipts pin content_hash here
//   scripts/sync_skill_pages.mjs     the bridge: a repo sync appends a version, never the reverse
// The generated SKILL_REGISTRY remains the read fallback so /api/skills never breaks when the
...
/**
 * Promote a candidate version to current (spec Phase 4 — the learning loop's one hard rule).
 * A candidate born from a failure earns promotion with EVIDENCE, never a vote and never its
 * author's say-so: two infrastructure-accepted task runs executed under the candidate version
 * (work_actions whose input/evidence carry the version pin and whose result is 'accepted'),
 * at least one of them a reproduction-kind task. The owner can force with a stated reason; the
 * force is recorded, never silent.
 */
