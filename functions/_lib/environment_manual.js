...
export function environmentManualMarkdown(catalog, { origin = 'https://miscsubjects.com' } = {}) {
  ...
  const lines = [
    ...
    '',
    'The live status board is a view sheet over the directory: `POST ' + absolute(origin, '/api/sheets') + '` `{"title":"TOOL STATUS","view":{"source":"directory","columns":["test_state","key","type","category","invocation","last_status","last_response","tested_at"],"limit":2000}}`; filter `test_state contains broken` for the red rows. The whole catalog is re-tested with `node scripts/test-directory-rows.mjs`.',
    '',
    '## Sheets by saying so',
    '',
    'A sheet can be made and changed by naming the change, in words, through the same verbs everything else uses. As directory rows they are `SHEET_LIST`, `SHEET_NEW` (title | source | columns | filter), `SHEET_ADD_COLUMN` (sheet_id | path), `SHEET_ADD_FILTER` (sheet_id | "field op value"), `SHEET_PIN` (sheet_id | directory/<KEY>/<field> | label), `SHEET_REMOVE_COLUMN`, `SHEET_VISIBILITY`, `SHEET_DELETE`. Because they are rows, a message to the build, an agent tag, the workbook\'s model panel and any MCP client call them the same way, and each call is one ledger receipt. Over REST the same verbs are `POST ' + absolute(origin, '/api/sheets') + '` `{"title","source","columns":"a,b,c","filter":"type = agent"}` and `POST ' + absolute(origin, '/api/sheets/<id>/columns|filters|pins:add|columns:remove') + '`.',
    '',
    '## Articles on a sheet, and a flow that reads inside a payload',
    '',
    '`articles` is a view source: slug, title, subject, published, body, meta (with JSON paths such as `meta.hero.url`). Typing in a title, subject, published, body or meta cell edits the article through its own write path; a body edit carries the body hash it read, so a concurrent edit is refused, never overwritten. Any view cell writes through `POST ' + absolute(origin, '/api/sheets/<id>/write') + '` `{"id","field","value"}`, routed by the view\'s source; the ledger stays read-only.',
    '',
    'In a flow, a step may read inside the previous step\'s JSON with a path: `$PREV.body.messages[0].content`, `$PREV.choices[0].message.content`, `$PREV[2].id`. A missing step is blank, not an error. This is how a transform between two tools is a row, not a coding session.',
    '',
