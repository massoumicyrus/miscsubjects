...
export async function onRequestGet(context) {
  ...
  for (const clause of LOOP_LAW_OBJECT.content.clauses) {
  ...
  const structured = JSON.stringify({
    ...
    "@type": "TechArticle",
    name: LOOP_LAW_OBJECT.identity.title,
    headline: LOOP_LAW_OBJECT.identity.title,
    description: LOOP_LAW_OBJECT.content.summary,
    url: "https://miscsubjects.com/a/loop-law",
    isPartOf: { "@type": "WebSite", name: "miscsubjects" },
    about: LOOP_LAW_OBJECT.content.clauses.map((clause) => clause.title),
  }).replace(/</g, "\\u003c");
  const ledgerThread = renderLedgerThread(
    "loop-law",
    await listComments(context.env, "loop-law", 200),
    {},
  );
  const html = `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
  <title>${LOOP_LAW_OBJECT.identity.title} — miscsubjects</title><meta name="description" content="${LOOP_LAW_OBJECT.content.summary}">
  <link rel="canonical" href="https://miscsubjects.com/a/loop-law"><link rel="alternate" type="application/json" href="https://miscsubjects.com/api/articles/loop-law"><link rel="alternate" type="text/markdown" href="https://miscsubjects.com/api/articles/loop-law?format=markdown"><script type="application/ld+json">${structured}</script>${fontLinks}<style>${designLawStyles()}${cssVarOverride(prof)}${PAGE_STYLE}</style></head><body>
  ${designLawHeader("loop-law")}
  <main>
    <header class="law-hero"><p class="eyebrow">Canonical Knowledge-Action Object · ${LOOP_LAW_OBJECT.version.current}</p><h1>The graph says<br>what happens next.</h1><p class="lead">${LOOP_LAW_OBJECT.content.thesis}</p></header>
    <section class="law-mandate"><p class="eyebrow">Resolve before acting</p>${LOOP_LAW_OBJECT.instructions.decision_mandate.map((line) => `<p>${line}</p>`).join("")}</section>
    <section class="law-sequence" aria-label="Complete loop obligations">${laws}</section>
    <section class="law-parallel"><p class="eyebrow">The live surfaces</p><h2>The queue, the health report, and the vault are one derivation.</h2><p><a href="/api/articles/next-acts">/api/articles/next-acts</a> ranks what to do; <a href="/api/articles/graph-lint">/api/articles/graph-lint</a> names what is structurally wrong; <a href="/api/articles/graph-links">/api/articles/graph-links</a> is the full edge set (embeds, typed links, wikilinks); <a href="/api/articles/obsidian-vault">/api/articles/obsidian-vault</a> projects the whole corpus as a wikilinked Obsidian vault whose index.md, lint.md, and next.md are these same surfaces as files. This law governs the models that act on them: <a href="/a/writing-law">writing</a>, <a href="/a/design-law">design</a>, <a href="/a/outreach-law">outreach</a>, and <a href="/a/skill-law">skill</a> law bind each surface's form.</p></section>
    <section class="law-source"><div><p class="eyebrow">One identity, many expressions</p><h2>The doctrine becomes a Skill.</h2><p>The page explains to a human. The Skill instructs a model. When a behavior is wrong, the owner points at the clause that allowed it; the clause is amended with the exhibit attached; every model that loads this object afterward inherits the fix. That is the repair path — documentation first, instance second.</p></div><ol>${facets}</ol></section>
    <section class="law-coda"><p>One derivation. Every act compounds the same graph.</p><details><summary>Traverse this object</summary>${Object.entries(
      LOOP_LAW_OBJECT.representations,
    )
      .map(
