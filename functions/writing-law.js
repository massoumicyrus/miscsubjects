...
export async function onRequestGet(context) {
  ...
  for (const clause of WRITING_LAW_OBJECT.content.clauses) {
  ...
  const structured = JSON.stringify({
    ...
    "@type": "TechArticle",
    name: WRITING_LAW_OBJECT.identity.title,
    headline: WRITING_LAW_OBJECT.identity.title,
    description: WRITING_LAW_OBJECT.content.summary,
    url: "https://miscsubjects.com/a/writing-law",
    isPartOf: { "@type": "WebSite", name: "miscsubjects" },
    about: WRITING_LAW_OBJECT.content.clauses.map((clause) => clause.title),
  }).replace(/</g, "\\u003c");
  const ledgerThread = renderLedgerThread(
    "writing-law",
    await listComments(context.env, "writing-law", 200),
    {},
  );
  const html = `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
  <title>${WRITING_LAW_OBJECT.identity.title} — miscsubjects</title><meta name="description" content="${WRITING_LAW_OBJECT.content.summary}">
  <meta property="og:image" content="https://miscsubjects.com/img/gen/arcads-law-writing-338ff49e-17fd-4adf-a9d3-01df2f6214f2.png"><meta name="twitter:card" content="summary_large_image"><meta name="twitter:image" content="https://miscsubjects.com/img/gen/arcads-law-writing-338ff49e-17fd-4adf-a9d3-01df2f6214f2.png"><link rel="canonical" href="https://miscsubjects.com/a/writing-law"><link rel="alternate" type="application/json" href="https://miscsubjects.com/api/articles/writing-law"><link rel="alternate" type="text/markdown" href="https://miscsubjects.com/api/articles/writing-law?format=markdown"><script type="application/ld+json">${structured}</script>${fontLinks}<style>${designLawStyles()}${cssVarOverride(prof)}${PAGE_STYLE}</style></head><body>
  ${designLawHeader("writing-law")}
  <main>
    <figure class="law-plate"><img src="https://miscsubjects.com/img/gen/arcads-law-writing-338ff49e-17fd-4adf-a9d3-01df2f6214f2.png" alt="A compositor's type case with lead letterpress sorts and a proof sheet on dark walnut" width="1536" height="1024" fetchpriority="high"><figcaption>Every sentence was set by hand once. The discipline survives the machinery.</figcaption></figure>
        <header class="law-hero"><p class="eyebrow">Canonical Knowledge-Action Object · ${WRITING_LAW_OBJECT.version.current}</p><h1>Opaque language<br>is hostility.</h1><p class="lead">${WRITING_LAW_OBJECT.content.thesis}</p></header>
    <section class="law-mandate"><p class="eyebrow">The test for every sentence</p>${WRITING_LAW_OBJECT.instructions.decision_mandate.map((line) => `<p>${line}</p>`).join("")}</section>
    <section class="law-sequence" aria-label="Complete writing obligations">${laws}</section>
    <section class="law-parallel"><p class="eyebrow">The parallel</p><h2>Design has this law already. Writing gets the same one.</h2><p>The <a href="/a/design-law">Design Law</a> removes every visual element that fails the existence test. This law removes every clause that fails it. Same axiom, same test, same repair: cut it, or rewrite it in plainer words with a concrete case.</p></section>
    <section class="law-source"><div><p class="eyebrow">One identity, many expressions</p><h2>The article becomes a Skill.</h2><p>The article explains to a human. The Skill tells a model how to write. The directory row exposes the live contract. Each expression uses the language its audience needs while preserving identity, meaning, version, relationships, and proof.</p></div><ol>${facets}</ol></section>
    <section class="law-coda"><p>One identity. Many typed expressions. Each optimized for its audience and role.</p><details><summary>Traverse this object</summary>${Object.entries(
      WRITING_LAW_OBJECT.representations,
    )
      .map(
