...
// Grok CLI turn log. Source of truth: grok_turns rows written by the Stop hook.
export async function onRequestGet(context) {
  ...
  if (url.searchParams.get('data') === '1') {
    ...
    const r = await env.DB.prepare(
      'SELECT id, ts, session, cwd, input_kind, user_input, user_input_chars, assistant_text, audit_verdict, audit_note, audit_engine, n_tools, tools_json, commands_json, files_json FROM grok_turns ORDER BY id DESC LIMIT ?'
    ).bind(limit).all();
...
<style>
.gx{max-width:1460px}
.gx-head{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:18px;align-items:end;border-bottom:1px solid var(--line);padding-bottom:16px;margin-bottom:16px}
.gx-title h1{font-size:26px;margin:0 0 4px}
.gx-sub{color:var(--ink-soft);font-size:13px;line-height:1.45;max-width:780px}
.gx-actions{display:flex;gap:8px;flex-wrap:wrap;justify-content:flex-end}
.gx-actions a,.gx-actions button{border:1px solid var(--line-strong);border-radius:6px;background:#fff;color:var(--ink);font-size:12px;font-weight:700;padding:8px 11px;text-decoration:none}
.gx-actions a:hover,.gx-actions button:hover{border-color:var(--accent);background:var(--accent-soft);text-decoration:none}
.gx-metrics{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:10px;margin:14px 0 16px}
.gx-stat{border:1px solid var(--line);border-radius:8px;background:#fff;padding:12px;min-height:76px}
.gx-stat b{display:block;font:800 24px/1 var(--mono);color:var(--ink)}
.gx-stat span{display:block;margin-top:7px;font-size:10px;font-weight:800;letter-spacing:.06em;text-transform:uppercase;color:var(--muted)}
.gx-controls{display:grid;grid-template-columns:minmax(220px,1fr) 170px 170px auto;gap:10px;align-items:center;border:1px solid var(--line);border-radius:8px;background:#fff;padding:10px;margin-bottom:16px}
.gx-controls input,.gx-controls select{width:100%}
.gx-toggle{display:flex;align-items:center;gap:7px;font-size:12px;font-weight:700;color:var(--ink-soft);white-space:nowrap}
.gx-toggle input{width:auto}
.gx-layout{display:grid;grid-template-columns:minmax(0,1fr) 310px;gap:16px;align-items:start}
...
@media(max-width:980px){.gx-head{grid-template-columns:1fr}.gx-actions{justify-content:flex-start}.gx-metrics{grid-template-columns:repeat(2,minmax(0,1fr))}.gx-controls{grid-template-columns:1fr}.gx-layout{grid-template-columns:1fr}.side{position:static}.turn-head{grid-template-columns:1fr}.turn-score{justify-content:flex-start;min-width:0}.detail-grid{grid-template-columns:1fr}}
</style>
<div class="gx">
  <section class="gx-head">
    <div class="gx-title">
      <h1>Grok CLI</h1>
      <div class="gx-sub">Grok CLI turn detail (grok_turns). All agents: <a href="/admin/agents">Agents</a> tab.</div>
    </div>
    <div class="gx-actions">
      <a href="/admin/ledger?cards=1&service=grok-cli">Ledger Cards</a>
      <a href="/admin/vault">Vault</a>
      <button type="button" id="refresh">Refresh</button>
    </div>
  </section>

  <section class="gx-metrics" id="metrics"></section>

  <section class="gx-controls">
    ...
    <label class="gx-toggle"><input type="checkbox" id="filesOnly"> Files only</label>
  </section>

  <section class="gx-layout">
    <div class="gx-list" id="list"><div class="loading">Loading...</div></div>
    <aside class="side">
      <div class="panel">
        <h2>Sessions</h2>
...
function stat(label,value){return '<div class="gx-stat"><b>'+esc(value)+'</b><span>'+esc(label)+'</span></div>';}
...
function renderRisks(){
  ...
  $('risks').innerHTML=risks.map(r=>
    '<a class="risk-link" href="/admin/ledger?cards=1&service=grok-cli&card_id=gr_'+encodeURIComponent(String(r.id))+'">'+
...
function renderTurn(r){
  ...
  const ledger='/admin/ledger?cards=1&service=grok-cli&card_id=gr_'+encodeURIComponent(String(r.id));
  return '<article class="turn '+(r.risk?'risk':'')+'" data-id="'+esc(r.id)+'">'+
    ...
    '<div class="turn-body">'+
      '<div class="detail-grid">'+
        ...
        '<div class="box"><div class="section-title">Links</div><div class="tool-row"><a href="'+ledger+'">Open ledger card</a></div><div class="tool-row"><a href="/admin/grok?data=1&limit=500">Raw JSON</a></div></div>'+
      '</div>'+
    '</div>'+
  '</article>';
}
...
async function load(){
  ...
  const r=await fetch('/admin/grok?data=1&limit=300');
...
load().catch(e=>{$('list').innerHTML='<div class="empty">Load failed: '+esc(e.message)+'</div>';});
  ...
  return new Response(shellHtml({ activeHref: '/admin/grok', title: 'Grok CLI', body: BODY }), {
