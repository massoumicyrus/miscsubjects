...
export async function onRequestGet(context) {
  ...
  for (const clause of LOGIC_LAW_OBJECT.content.clauses) {
  ...
  const structured = JSON.stringify({
    ...
    "@type": "TechArticle",
    name: LOGIC_LAW_OBJECT.identity.title,
    headline: LOGIC_LAW_OBJECT.identity.title,
    description: LOGIC_LAW_OBJECT.content.summary,
    url: "https://miscsubjects.com/a/logic-law",
    isPartOf: { "@type": "WebSite", name: "miscsubjects" },
    about: LOGIC_LAW_OBJECT.content.clauses.map((clause) => clause.title),
  }).replace(/</g, "\\u003c");
  const ledgerThread = renderLedgerThread(
    "logic-law",
    await listComments(context.env, "logic-law", 200),
    {},
  );
  const html = `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
  <title>${LOGIC_LAW_OBJECT.identity.title} — miscsubjects</title><meta name="description" content="${LOGIC_LAW_OBJECT.content.summary}">
  <meta property="og:image" content="https://miscsubjects.com/img/gen/arcads-law-logic-6fa10350-4e9d-4dcf-8619-f25f6914da82.png"><meta name="twitter:card" content="summary_large_image"><meta name="twitter:image" content="https://miscsubjects.com/img/gen/arcads-law-logic-6fa10350-4e9d-4dcf-8619-f25f6914da82.png"><link rel="canonical" href="https://miscsubjects.com/a/logic-law"><link rel="alternate" type="application/json" href="https://miscsubjects.com/api/articles/logic-law"><link rel="alternate" type="text/markdown" href="https://miscsubjects.com/api/articles/logic-law?format=markdown"><script type="application/ld+json">${structured}</script>${fontLinks}<style>${designLawStyles()}${cssVarOverride(prof)}${PAGE_STYLE}</style></head><body>
  ${designLawHeader("logic-law")}
  <main>
    <figure class="law-plate"><img src="https://miscsubjects.com/img/gen/arcads-law-logic-6fa10350-4e9d-4dcf-8619-f25f6914da82.png" alt="A brass balance scale on dark stone in low raking light, one pan empty" width="1536" height="1024" fetchpriority="high"><figcaption>Change must earn its existence. The empty pan is the default: no move at all.</figcaption></figure>
        <header class="law-hero"><p class="eyebrow">Canonical Knowledge-Action Object · ${LOGIC_LAW_OBJECT.version.current}</p><h1>Change must earn<br>its existence.</h1><p class="lead">${LOGIC_LAW_OBJECT.content.thesis}</p></header>
    <section class="law-mandate"><p class="eyebrow">Resolve before any move</p>${LOGIC_LAW_OBJECT.instructions.decision_mandate.map((line) => `<p>${line}</p>`).join("")}</section>
    <section class="law-sequence" aria-label="Complete operational logic">${laws}</section>
    <section class="law-parallel"><p class="eyebrow">The selector</p><h2>Writing and design govern the move. This law selects it.</h2><p>The <a href="/a/writing-law">Writing Law</a> asks whether the sentence deserves to exist. The <a href="/a/design-law">Design Law</a> asks whether the element deserves to exist. This law asks the question underneath both: whether the change deserves to exist at all. It decides which laws fire on the present turn — and when the correct move is none.</p></section>
    <section class="law-source"><div><p class="eyebrow">One identity, many expressions</p><h2>The article becomes a Skill.</h2><p>The article explains to a human. The Skill tells a model how to decide. The directory row exposes the live contract. Each expression uses the language its audience needs while preserving identity, meaning, version, relationships, and proof.</p></div><ol>${facets}</ol></section>
    <section class="law-coda"><p>One identity. Many typed expressions. Each optimized for its audience and role.</p><details><summary>Traverse this object</summary>${Object.entries(
      LOGIC_LAW_OBJECT.representations,
    )
      .map(
