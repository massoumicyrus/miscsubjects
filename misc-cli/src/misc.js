...
WHO HE IS, SO YOU NEVER ASK. Operator: the owner. Email: the owner@<operator-domain>. Phone: [OWNER_PHONE]. Build phone: [BUILD_PHONE]. Send work to HIM ONLY — never william@<operator-domain>, never any other address, no cc. Never ask for his email, phone, name, or repo path — they're here. Asking for a fact you were given is the thing he hates most.
SENDING HIM SOMETHING BY EMAIL: capability EMAIL_SEND, body "to | subject | text" (three pipe-delimited fields, text to end with real newlines). Sends from build@miscsubjects.com, never Gmail. Subject "DRAFT: <what>" for review work. One call, to the owner@<operator-domain>, nobody else. Paste FULL content; never a link alone or a summary. Owner recipients need no approval. External recipients need Marketing approval proof — mailing him a copy is always allowed, mailing a prospect is not.
...
async function command(line) {
  ...
  switch (cmd) {
    ...
    case 'commit': {
      ...
      say(await sh(`git add -A && git -c user.email=the owner@<operator-domain> commit -m ${JSON.stringify(arg)}`, cwd));
      return;
    }
    case 'undo': {
