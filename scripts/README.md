...
Canonical deploy path for the Pages build: apply the newest D1 migration to `miscsubjects-content` first, then deploy `public/` to `miscsubjects-pages`. Pass explicit `.sql` files to apply those instead, or `--no-migrations` for a code-only redeploy.
