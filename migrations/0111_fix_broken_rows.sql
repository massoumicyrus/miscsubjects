...
{"cmd":"sh","args":["-lc","cd /Users/owner/miscsubjects-pages && npx wrangler pages deploy public --project-name loop-safe-miscsubjects --commit-dirty=true $1+"],"timeout":600000}'
WHERE key = 'WRANGLER_DEPLOY';

-- 2. Fix WRANGLER_D1_EXPORT: same swap issue
UPDATE directory SET
  target = 'POST https://agent.cannibal.capital/exec',
  content = '# WHAT: Export the production D1 (loop-content-spine) to a timestamped .sql file on the Mac for backup. Returns the file path and size of the latest dump
...
{"cmd":"sh","args":["-lc","cd /Users/owner/miscsubjects-pages && npx wrangler d1 export loop-content-spine --remote --output /tmp/d1-backup-$(date +%s).sql $1+ && ls -la /tmp/d1-backup-*.sql | tail -1"],"timeout":300000}'
...
{"cmd":"sh","args":["-lc","cd /Users/owner/miscsubjects-pages && timeout 30 npx wrangler pages deployment tail --project-name loop-safe-miscsubjects --format=pretty $1+ 2>&1 | head -200"],"timeout":40000}'
