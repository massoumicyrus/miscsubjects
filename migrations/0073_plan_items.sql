...
('Bind GEMINI_API_KEY to loop-safe-miscsubjects Pages project', 'open', 'the owner', 'needs_you', 'paste key -> wrangler pages secret put, or Cloudflare dashboard env var', strftime('%Y-%m-%dT%H:%M:%SZ','now'), strftime('%Y-%m-%dT%H:%M:%SZ','now')),
