...
DELETE FROM directory_tests WHERE kind='e2e' AND note IN (
  'audit drop copies payload with articles',
  'floating admin audit copies complete payload',
  'bounded self-explaining audit token drop',
  'audit drop has no obligational language'
);
INSERT INTO directory_tests (key,kind,args,expect_kind,expect_value,note,expected_text,tier) VALUES
('ROUTER','e2e','In the floating Owner Tap & Go, press Audit this build. What kind of object gets copied?','reply_ok','bounded|token DROP|read capability|evidence|retriev|not embedded','bounded self-explaining audit token drop','The floating control mints and copies a bounded self-explaining read-token DROP. The DROP carries the audit task, evidence traversal, comparison method, response shape, and failure states. Article bodies and the full evidence archive remain retrievable outside the model context.','8'),
('ROUTER','e2e','What writing register does the build-audit token DROP use?','reply_ok','mechanism|state|RESPONSE SHAPE|no|required|must|need|should','audit drop has no obligational language','The audit DROP uses mechanism and acceptance-state language. Its headings include RESPONSE SHAPE, EVIDENCE FLOW, and FAILURE STATES. It contains no REQUIRED ANSWER, must, need, should, ensure, or audience-directed compliance commands.','8');
