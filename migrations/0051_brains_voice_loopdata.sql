...
-- ─────────────── Loop data platform (api.<tenant-domain>) read access ───────────────
('TENANT_GET', 'http', 'GET https://api.<tenant-domain>/$1', '',
'# GET any path on the loop data platform (api.<tenant-domain>, the loop-api-worker). Arg: the path after the host, e.g. "v4/health" or "2chat/contacts". Returns the raw JSON. when_to_use: the owner asks for data from the loop platform / <tenant-domain>. If a path needs auth and returns 401, ask the owner which header/token it expects and EDIT_ROW to add it.',
'loopdata', NULL, 45, strftime('%Y-%m-%dT%H:%M:%fZ','now')),

('TENANT_POST', 'http', 'POST https://api.<tenant-domain>/$1', '',
'# POST to a path on the loop data platform (api.<tenant-domain>). Args: path|json_body (body is everything after the first pipe). when_to_use: the owner asks to send/trigger something on the loop platform. Add auth headers via EDIT_ROW when the endpoint needs them.
$2+',
...
INSERT INTO docs (slug, title, body, updated_at) VALUES (
  ...
  'This build is Cloudflare Pages project miscsubjects-miscsubjects serving miscsubjects.com. Stack: Pages Functions (the functions/ dir = file-routed Workers), D1 (SQLite — DB=miscsubjects-content holds the directory+docs+tasks+pages; LEDGER=miscsubjects-events holds the events audit log), KV (loop_content_kv, binding KV — directory:snapshot, repo:snapshot:current, convo memory), R2 (miscsubjects-ledger, binding R2 — generated images img/gen/, audio img/aud/, uploads img/up/, big ledger payloads events/), Workers AI (binding AI), AI Gateway (gateway "default", gw:provider/model ids).
...
Deploy this build: [LOCAL_EXEC]cd /Users/owner/miscsubjects-pages && npx wrangler pages deploy public --project-name miscsubjects-miscsubjects --commit-dirty=true[/LOCAL_EXEC] (ALWAYS cd first — deploying from $HOME ships an empty site). Apply a D1 migration: write the .sql, then npx wrangler d1 execute miscsubjects-content --remote --file <path>. Read live docs the owner needs with web search (developers.cloudflare.com).',
