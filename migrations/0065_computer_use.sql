...
('LOCAL_APPS','http','POST https://agent.<bridge-domain>/exec','headers:{"x-terminal-key":"$TERMINAL_KEY"}','# WHAT: List running GUI apps on the Mac (foreground processes).
...
INSERT OR REPLACE INTO directory (key,type,target,auth,content,category,runner,planner_rank,planner_visible,enabled,updated_at) VALUES
('LOCAL_FRONTMOST','http','POST https://agent.<bridge-domain>/exec','headers:{"x-terminal-key":"$TERMINAL_KEY"}','# WHAT: Name of the frontmost (active) app on the Mac.
...
INSERT OR REPLACE INTO directory (key,type,target,auth,content,category,runner,planner_rank,planner_visible,enabled,updated_at) VALUES
('LOCAL_WINDOWS','http','POST https://agent.<bridge-domain>/exec','headers:{"x-terminal-key":"$TERMINAL_KEY"}','# WHAT: List window titles of the frontmost app.
...
INSERT OR REPLACE INTO directory (key,type,target,auth,content,category,runner,planner_rank,planner_visible,enabled,updated_at) VALUES
('LOCAL_ACTIVATE','http','POST https://agent.<bridge-domain>/exec','headers:{"x-terminal-key":"$TERMINAL_KEY"}','# WHAT: Bring an app to the front (focus it).
...
INSERT OR REPLACE INTO directory (key,type,target,auth,content,category,runner,planner_rank,planner_visible,enabled,updated_at) VALUES
('LOCAL_KEYSTROKE','http','POST https://agent.<bridge-domain>/exec','headers:{"x-terminal-key":"$TERMINAL_KEY"}','# WHAT: Type text into the focused field on the Mac (System Events keystroke).
...
INSERT OR REPLACE INTO directory (key,type,target,auth,content,category,runner,planner_rank,planner_visible,enabled,updated_at) VALUES
('LOCAL_KEYCODE','http','POST https://agent.<bridge-domain>/exec','headers:{"x-terminal-key":"$TERMINAL_KEY"}','# WHAT: Send a macOS key code to the focused app (36=return 53=esc 48=tab 123-126=arrows).
...
INSERT OR REPLACE INTO directory (key,type,target,auth,content,category,runner,planner_rank,planner_visible,enabled,updated_at) VALUES
('LOCAL_UI_SNAPSHOT','http','POST https://agent.<bridge-domain>/exec','headers:{"x-terminal-key":"$TERMINAL_KEY"}','# WHAT: Accessibility snapshot of the frontmost window — role+name+description of each top-level UI element. Semantic, not pixels. The basis for LOCAL_UI_CLICK.
...
INSERT OR REPLACE INTO directory (key,type,target,auth,content,category,runner,planner_rank,planner_visible,enabled,updated_at) VALUES
('LOCAL_UI_CLICK','http','POST https://agent.<bridge-domain>/exec','headers:{"x-terminal-key":"$TERMINAL_KEY"}','# WHAT: Click a UI element by NAME in the frontmost app (semantic, not blind x/y). Pair with LOCAL_UI_SNAPSHOT to find names.
