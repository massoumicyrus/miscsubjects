-- De-sprawl Tier 2: collapse exec-alias families to generic LOCAL_EXEC.
-- Deletes WRANGLER_* (283), GH_* (192), CLASP_* (29) = 504 rows; all are 'run a shell command'.
-- ROUTER prompt rewritten first (only it named any of these: 5 WRANGLER refs) so no dead-key window.
-- Reversible: rows preserved in directory.snapshot.json; ROUTER original in prompts/ROUTER.v_pre_desprawl.backup.md
...
' WHERE key='ROUTER';
DELETE FROM directory WHERE key GLOB 'WRANGLER_*';
DELETE FROM directory WHERE key GLOB 'GH_*';
DELETE FROM directory WHERE key GLOB 'CLASP_*';
