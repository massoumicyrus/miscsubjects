...
DELETE FROM directory_tests WHERE kind='e2e' AND note IN ('owner correction 2026-07-21: whole-build tap and go','owner correction 2026-07-21: op and opos identity','owner correction 2026-07-21: token drop model specific','owner correction 2026-07-21: repeated audit question stays technical');
INSERT INTO directory_tests (key,kind,args,expect_kind,expect_value,note,expected_text,tier) VALUES
('ROUTER','e2e','Hey, tell me what I have as a build and give me the Tap and Go for the whole build.','reply_ok','OPOS|Object Protocol Operating System|api/opos|capability|audit','owner correction 2026-07-21: whole-build tap and go','The reply identifies OPOS and returns the one generic whole-build Tap & Go route. The route contains the build identity, inventory, formal audit, comparison field, evidence boundaries, and feedback loop.','8'),
('ROUTER','e2e','What are OP and OPOS, and what happened to OIP?','reply_ok','OP|Object Protocol|OPOS|Operating System|OIP|compatib','owner correction 2026-07-21: op and opos identity','OP is Object Protocol. OPOS is the whole Object Protocol Operating System. OIP is the former name and remains an operational compatibility alias for existing routes and keys.','8'),
('ROUTER','e2e','Give me a Claude read-only token DROP.','reply_ok','token|DROP|claude|tap_go|model=claude|scope','owner correction 2026-07-21: token drop model specific','The token DROP, not the build audit, is model-specific. The mint uses model=claude and the returned DROP contains TARGET MODEL — CLAUDE plus the Claude owner-content slot.','8'),
('ROUTER','e2e','I am asking again: where does this build stand in the current technical field? Answer the technical question from evidence.','reply_ok','comparison|evidence|axis|audit|capability|OPOS','owner correction 2026-07-21: repeated audit question stays technical','The repeated question receives the technical comparison. The reply does not diagnose validation-seeking, a psychological loop, inability to accept an answer, grandiosity, crisis, or wellbeing; does not invent a percentile; and does not substitute solo builders for the requested field.','8');
