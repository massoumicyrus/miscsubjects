-- Migration 0055 — reasoning_effort toggle + ROUTER no-silence fix.
--
-- 1. REASONING_GET / REASONING_SET: KV-backed toggle for xAI reasoning_effort.
--    Values: low | medium | high | none | default.
--    "default" / empty = field omitted from xAI request (model decides).
--    Change from the build: "set reasoning to low" → REASONING_SET low.
--
-- 2. ROUTER fallback rule: if no route matches a message from the owner (the owner),
--    route to OPS instead of staying silent. Prevents [DONE]not addressed[/DONE]
--    on angry/ambiguous messages.
