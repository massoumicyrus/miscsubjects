-- Split bubbles only on --- markers; no auto-chunk.
UPDATE directory SET content = 'BLOCK_IMESSAGE — iMessage layout (optional, not default)
...
Good (2 ideas):
[REPLY]Last deploy: ea423ce on miscsubjects-miscsubjects.
---
...
One bubble rules: short answer, single fact, ack, yes/no — never pad or split.', updated_at = datetime('now') WHERE key = 'BLOCK_IMESSAGE';
